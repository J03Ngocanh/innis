...
<form id="thanhtoan" action="<?= WEBROOT ?>giohang/tienhanhthanhtoangiohang/<?php echo $id_giohang ?>" method="POST">
<div class="container">
    <div class="left-section">
        ...
        <?php
        $tongtien=0;
        while($data=mysqli_fetch_array($thongtinsanpham)){
        ?>
        <div class="order-item">
        <img style="width:200px; height:auto;" src="<?php echo WEBROOT . 'public/img/'.$data['hinhanh']  ?>" alt="Sản phẩm">
            <div class="order-info">
                <p><strong><?php echo htmlspecialchars($data['tensanpham']); ?></strong></p>
                <p>Giá: <span><?php echo number_format($data['giagoc']); ?>đ</span></p>
                <div class="quantity">
                     <label for="soluong<?php echo $data['masanpham']; ?>">Số lượng:</label>
                     <input type="number" class="soluong" id="soluong<?php echo $data['masanpham']; ?>" name="soluong[<?php echo $data['masanpham']; ?>]" value="<?php echo $data['soluong']; ?>" min="1" readonly>
                </div>
            </div>
        </div>
        <?php 
        $tongtien += $data['giagoc'] * $data['soluong'];
        } ?>
        <label for="tongTien">Tổng tiền:</label>
        <input type="text" id="tongTien"  value="<?php echo number_format($tongtien); ?>đ" readonly>
